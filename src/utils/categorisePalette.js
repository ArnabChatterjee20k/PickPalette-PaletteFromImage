export default function categorizeColors(colors) {
    ...
    // Assign duplicate colors if any category is null
    const uniqueColors = new Set(Object.values(categorizedColors));
    const allCategories = ['primary', 'secondary', 'tertiary', 'accent'];
  
    for (const category of allCategories) {
      if (categorizedColors[category] === null) {
        for (const color of sortedColors) {
          if (!uniqueColors.has(color)) {
            categorizedColors[category] = color;
